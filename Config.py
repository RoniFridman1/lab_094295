...
class Config:
    def __init__(self):
        ...
        self.leaning_rate = 1e-4
        self.MODELS = ["vgg16"]  # "resnet18", "vgg16"
        self.DATA_DIR = "chest_xray"
        self.SAMPLING_METHODS = ["random"]  # "uncertainty", "entropy", "random", "pca_then_kmeans"
        self.ACTIVE_LEARNING_ITERATIONS = 5
        self.MODEL_TRAINING_EPOCHS = 3
        self.SAMPLES_PER_ITERATION = 1
        self.TOTAL_TRAINING_SAMPLES = 5216  # Max is 5216
        self.TRAIN_LABELED_UNLABELED_RATIO = (0.1, 0.9)  # for baseline use (0.999, 0.001)
        ...
        if "pca_then_kmeans" in self.SAMPLING_METHODS:
            ...
            err1 = prefix + f"Make sure that the number of \nlabeled training " \
                f"samples is greater or equal to the number of samples per iteration.\n" + \
            ...
            err3 = prefix + f"Number of cluster should be at least the number of samples per iteration,\n" + \
                f"Since we choose one sample of each the top n_samples_per_iteration clusters."

            assert (self.TOTAL_TRAINING_SAMPLES * self.TRAIN_LABELED_UNLABELED_RATIO[0] >= self.SAMPLES_PER_ITERATION), err1
